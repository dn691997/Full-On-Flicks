...
var getTitleId = function (searchTerm) {
  var apiURL =
    "https://imdb-api.com/en/API/SearchTitle/k_ix11kdvq/" + searchTerm;
        ...
        if (data.actorList[0]) {
          for (var i = 0; i < 6; i++) {
            if (data.actorList[i]) {
              ...
              castAnchorEl.innerHTML =
                "<figure><img src='" +
                castMemberImage +
                "' alt='" +
                castMemberName +
                "' width='128px'/><figcaption>" +
                formattedName +
                "</figcaption></figure>";
...
var getRandomMovie = function (number) {
  fetch("https://imdb-api.com/en/API/Top250Movies/k_ix11kdvq").then(function (
    response
  ) {
    if (response.ok) {
      response.json().then(function (data) {
...
var getRandomTV = function (number) {
  fetch("https://imdb-api.com/en/API/Top250TVs/k_ix11kdvq").then(function (
    response
  ) {
    if (response.ok) {
      response.json().then(function (data) {
...
var getTitleBtns = function (titleId) {
  var apiUrl =
    ...
    titleId +
    "/sources/?apiKey=XtDVqWrPdNevd0HMRCFzh8nSvnBpjdpVoazNV42f";
    ...
    if (response.ok) {
      response.json().then(function (data) {
        ...
        if (buttonsContainer.innerHTML == "") {
          ...
          sorryDiv.textContent =
            "Sorry, we were unable to find a streaming service for this title. 😞";
